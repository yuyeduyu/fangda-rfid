...
public class DiaoBoInfoActivity extends AppCompatActivity {

    @BindView(R.id.toolbar)
    Toolbar mToolbar;
    DiaoBoResultBean.DiaoBoBean data;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_diaobo_info);
        ButterKnife.bind(this);
        initTool();
        Intent intent = getIntent();
        data = (DiaoBoResultBean.DiaoBoBean) intent.getSerializableExtra("data");
        if (data == null) {
            Toast.makeText(DiaoBoInfoActivity.this, "获取数据出错，请退出界面重新进入", Toast.LENGTH_SHORT).show();
        } else {
            shownInfo();
        }
    }
